...
The current `undo-v15` r1 development candidate extends the cumulative series through table API inheritance, table text-cursor Undo, exact existing-master layout selection, observable mixed-script text formatting, native object creation/duplication and metric-driven text-layout invalidation in `0027`–`0033`. `sourceCandidateReady` stays false until the source-level native suites pass. Runtime promotion remains separate and requires the integrated image, browser, OOXML, visual and PowerPoint gates described in the parent README.
