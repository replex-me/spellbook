...
The current `undo-v14` r1 source candidate extends the cumulative series through table API inheritance, table text-cursor Undo, exact existing-master layout selection, observable mixed-script text formatting and native object creation/duplication in `0027`–`0032`. `sourceCandidateReady` records that the source-level native suites passed. Runtime promotion remains separate and requires the integrated image, browser, OOXML, visual and PowerPoint gates described in the parent README.
