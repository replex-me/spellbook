...
The current `undo-v16` r1 development candidate extends the cumulative series through table API inheritance, table text-cursor Undo, exact existing-master layout selection, observable mixed-script text formatting, native object creation/duplication, metric-driven text-layout invalidation and sparse-master-safe slide insertion in `0027`–`0034`. `sourcePatchSeriesReady` records clean-source build admission; `sourceCandidateReady` stays false until the integrated source-level native suites pass. Runtime promotion remains separate and requires the browser, OOXML, visual and PowerPoint gates described in the parent README.
