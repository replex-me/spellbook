...
The current `undo-v13` r8 source candidate extends the cumulative series through table API inheritance, table text-cursor Undo, exact existing-master layout selection and mixed-script text formatting in `0027`–`0030`. `sourceCandidateReady` records that the source-level native suites passed. Runtime promotion remains separate and requires the integrated image, browser, OOXML, visual and PowerPoint gates described in the parent README.
