...
if (!url || !reportPath)
  throw new Error(
    "Usage: node probe-uno-animation-timing-reopen.mjs URL REPORT.json",
  );
...
if (!expected?.slides || !expected?.masters)
  throw new Error("The animation-timing report has no persistence data.");
...
try {
  ...
  if (!patchLevelMatch || Number(patchLevelMatch[1]) < 8)
    throw new Error(
      `Expected undo-v8 or newer, got ${observed.engine?.patchLevel}.`,
    );
  assertDocumentPersistenceDelta(
    report,
    observed,
    "Saved PPTX did not preserve the exact animation timing state.",
  );
