...
if (!url || !reportPath)
  throw new Error(
    "Usage: node probe-uno-object-interaction-reopen.mjs URL REPORT.json",
  );
...
if (!report.persistenceExpected?.slides || !report.persistenceExpected?.masters)
  throw new Error("The object-interaction report has no persistence data.");
...
try {
  ...
  if (!patchLevelMatch || Number(patchLevelMatch[1]) < 18)
    throw new Error(
      `Expected undo-v18 or newer, got ${observed.engine?.patchLevel}.`,
    );
  assertDocumentPersistenceDelta(
    report,
    observed,
    "Saved PPTX did not preserve the exact object interaction state.",
  );
