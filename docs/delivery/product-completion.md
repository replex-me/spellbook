...
| Public infrastructure, performance and safety |      10 |      4 | Compose/local auth/storage, signed WOPI verification, backup/isolated restore, restart recovery, private GCP validation, central control-plane contracts and a crash-tolerant browser OPFS journal provide the base          | Public signup/deletion, direct client storage and server reconciliation, product-integrated OPFS recovery, multi-user isolation, load/backpressure/autoscaling, billing/entitlement, clean native host/TLS/disk/interrupted-job exercises |
