...
The native mutation model currently classifies 98 operations, of which 97 bounded operations are exposed to the AI tool schema; the per-shape printable flag is excluded because PPTX cannot preserve it. 63 have passed the live runtime path through native Undo/Redo, save/reopen, scoped package-change checks and PowerPoint reopening. The other 34 are implemented against the cumulative `undo-v24` engine candidate and require final runtime validation; the matching `browser-undo-v12` candidate covers sections, chart formatting, document design, connectors/freeform, shape and text formatting, comments, animation lifecycle, images/media, SmartArt, Fontwork, 3D, equations and reading order. Those 34 are not release-verified until the one final compiled candidate passes the shared 15-scenario runtime suite and the downstream visual and PowerPoint checks. Every execution is rebound to the current document permission and live engine patch level. “Implemented in source,” “available to the model” and “release-verified for autonomous execution” remain separate states.
