...
The native mutation model currently classifies 98 operations. It explicitly excludes the per-shape printable flag because PPTX cannot preserve it and exposes the remaining 97 bounded operations to the AI tool schema. The previous immutable `undo-v18` runtime verified 63 operations through native Undo/Redo, save/reopen, scoped package-change checks and PowerPoint reopening. The cumulative `undo-v24` and `browser-undo-v11` source candidates implement the other 34 operations across sections, chart formatting, document design, connectors/freeform, shape and text formatting, comments, animation lifecycle, images/media, SmartArt, Fontwork, 3D, equations and reading order. Those 34 are not release-verified until the one final compiled candidate passes the shared 15-scenario runtime suite and the downstream visual and PowerPoint checks. Every execution is rebound to the current document permission and live engine patch level. “Implemented in source,” “available to the model” and “release-verified for autonomous execution” remain separate states.
