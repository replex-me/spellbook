...
The native mutation model currently classifies 64 operations, of which 63 bounded operations are exposed to the AI tool schema. 28 have passed the live runtime path; 34 are implemented against the cumulative `undo-v18` engine candidate but are rejected at execution until that exact engine passes its remaining release gates; image crop remains under runtime validation; and printable-state mutation is explicitly excluded from PPTX. Every execution is rebound to the current document permission and live engine patch level. “Implemented in LibreOffice,” “available to the model” and “release-verified for autonomous execution” are intentionally separate states.
